...
function semantic_title( $sep = '&mdash;' ) {
	...
	elseif ( is_author() ) : wp_title( __( 'Archives for', 'redline' ), true, 'left' );
	echo ( ' | ' ); bloginfo( 'name' );
	echo ( ' - ' );
	echo bloginfo( 'description' );
	  
	elseif ( is_archive() ) : wp_title( __( 'Archives for', 'redline' ), true, 'left' );
	echo ( ' | ' ); bloginfo( 'name' );
	echo ( ' - ' );
	echo bloginfo( 'description' );
	
	elseif ( is_search() ) : wp_title( __( 'Search Results', 'redline' ), true, 'left' );
	echo ( ' | ' ); bloginfo( 'name' );
	echo ( ' - ' );
	echo bloginfo( 'description' );
	
	elseif ( is_404() ) : wp_title( __( 'Error 404!', 'redline' ), true, 'left' );
	echo ( ' | ' ); bloginfo( 'name' );
	echo ( ' - ');
	echo bloginfo( 'description' );
...
function semantic_body( $classes = array() ) {
	...
	elseif ( is_page() ) {
		...
		$classes[] = 'page-author-' . sanitize_title_with_dashes( strtolower( get_the_author() ) );
...
function semantic_gallery( $attr ) {
	...
	$output = apply_filters( 'gallery_style', (string) "\n" . '<div class="gallery gallery-row">', 19 ); // Available filter: gallery_style
